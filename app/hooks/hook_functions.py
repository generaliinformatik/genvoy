            ...
            try:
                if valid_reference != "":
                    ...
                    if valid_reference[0] == "!":
                        ...
                        logging.debug("reading external reference file %s" % (valid_reference))
                        try:
                            with open(valid_reference, 'r') as reference_file:
                                reference_list.append(reference_file.read())
                        except:
                            logging.debug("invalid external file reference: !%s" % (valid_reference))
                            pass
                    elif valid_reference[0] == "@":
                        # internal file reference initiated by !
                        valid_reference = valid_reference[1:]
                        logging.debug("reading reference {%s/%s}" % (str(alias_reference), valid_reference))
                        try:
                            # valid reference found
                            dictionary_entry = ""
                            dictionary_entry = payload_config[str(alias_reference)][valid_reference]
                            reference_list.append(dictionary_entry)
                            logging.debug("valid reference: {%s/%s} => %s" % (str(alias_reference), valid_reference, dictionary_entry))
                        except:
                            # no valid reference found
                            logging.debug("invalid reference: {%s/%s}" % (str(alias_reference), valid_reference))
                            pass
                    else:
                        logging.debug("no further reference given {%s/%s}", eventtype, str(alias_reference))
                        reference_list.append(valid_reference)
            except:
                # no valid webhook dictionary entry, aborting
                logging.debug("no valid %s reference found: %s:%s" %  (str(alias_reference), eventtype, valid_reference))
                pass
    except:
